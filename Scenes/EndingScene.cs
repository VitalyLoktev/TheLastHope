...
namespace Game
{
    internal class EndingScene : IScene
    {
        ...
        private void ShowEndingTitle(GameContext context)
        {
            switch (context.Ending)
            {
                ...
                case EndingType.Secret:
                    context.Printer.Print(
                        "=== ПОЛУЧЕНА СЕКРЕТНАЯ КОНЦОВКА ===\n",
                        TextRole.SecretEnding,
                        TextSpeed.Normal,
                        context.InstantText
                    );
                    break;
